...
CHATWOOT_HUB_URL="https://hub.2.alloochat.ai/events"
...
Join the community at https://alloochat.ai/community?utm_source=cwctl
***************************************************************************
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.alloochat.ai/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
https://www.alloochat.ai/docs/deployment/deploy-chatwoot-in-linux-vm?utm_source=cwctl
...
Get help, https://alloochat.ai/community?utm_source=cwctl
