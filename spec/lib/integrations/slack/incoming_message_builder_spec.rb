  ...
  describe '#perform' do
    ...
    context 'when message creation' do
      ...
      it 'does not create message if message already exists' do
        ...
        expect(conversation.messages.last.content).to eql('this is test https://alloochat.ai Hey @Sojan Test again')
      end
      ...
      let(:link_shared) do
        {
          team_id: 'TLST3048H',
          api_app_id: 'A012S5UETV4',
          event: link_shared_event.merge({ links: [{ url: "https://qa.alloochat.ai/app/accounts/1/conversations/#{conversation.display_id}",
