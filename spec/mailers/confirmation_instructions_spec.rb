...
RSpec.describe 'Devise::Mailer' do
  describe 'notify' do
    ...
    it 'has the correct header data' do
      expect(mail.reply_to).to contain_exactly('accounts@aloochat.ai')
    ...
    it 'does not refer to the inviter and their account' do
      expect(mail.body).not_to match('has invited you to try out AlooChat!')
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        expect(mail.body).to match(
          "#{CGI.escapeHTML(inviter_val.name)}, with #{CGI.escapeHTML(account.name)}, has invited you to try out AlooChat."
        )
