...
RSpec.describe '/api/v1/widget/conversations/toggle_typing', type: :request do
  ...
  def conversation_params
    {
      ...
      contact: {
        name: 'contact-name',
        email: 'contact-email@aloochat.ai',
  ...
  describe 'POST /api/v1/widget/conversations' do
    it 'creates a conversation with correct details' do
      ...
      expect(json_response['contact']['email']).to eq 'contact-email@aloochat.ai'
    ...
    it 'does not update the name if the contact already exist' do
      existing_contact = create(:contact, account: account, email: 'contact-email@aloochat.ai')
  ...
  describe 'POST /api/v1/widget/conversations/set_custom_attributes' do
    let(:params) { { website_token: web_widget.website_token, custom_attributes: { 'product_name': 'AlooChat' } } }
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should have "product_name" key with value "AlooChat"
        expect(conversation.custom_attributes).to include('product_name' => 'AlooChat')
      end
    end
  end
  ...
  describe 'POST /api/v1/widget/conversations/destroy_custom_attributes' do
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        conversation.custom_attributes = { 'product_name': 'AlooChat' }
        conversation.save!
        expect(conversation.custom_attributes).to include('product_name' => 'AlooChat')
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should not have "product_name" key with value "AlooChat"
        expect(conversation.custom_attributes).not_to include('product_name' => 'AlooChat')
      end
    end
  end
end
