...
RSpec.describe SlackUnfurlJob do
  ...
  let(:link_shared) do
    {
      ...
      event: {
        ...
        channel: 'G01354F6A6Q',
        links: [{
          url: "https://qa.aloochat.ai/app/accounts/#{hook.account_id}/conversations/#{conversation.display_id}",
  ...
  context 'when the calls the slack unfurl job' do
    ...
    context 'when another account URL is shared' do
      before do
        link_shared[:event][:links][0][:url] = 'https://qa.aloochat.ai/app/accounts/123/conversations/123'
      end
